...
/*
Development NOTES:
Read what resolved: TSNav.config(). Inspect defaults: TSNav.defaults.

 Auto-init still runs on DOM-ready using layers 1+2, so the global header call works with zero JS. Module instances are created once (guarded), so calling init() again to apply a local override won't double-wire. The old window.TS_NAV_DISABLE_CSS_BRIDGE / TS_NAV_DISABLE_MEDIA_TOKENIZE booleans still work — they're folded into the new config. Each feature's gate lives at its call site (if (!CONFIG.logoFit) return, etc.), and the header documents how to add a new configurable feature in the same pattern.

Logo fitter. The function was already in the file but was skipping showcases. It now runs everywhere by default, gated by two flags: logoFit (run it at all) and logoFitInShowcase (also fit harness/demo logos — defaults to true). Set logoFitInShowcase: false if you ever want to skip .ts-nav--static / .ts-harness-stage logos.
Config system (TS_CONFIG). Every special feature, class, function, and selector bucket is now toggleable/retargetable on init, merged in three layers (later wins):

Built-in defaults
Global "header call" — window.TSNavConfig = {...} set in the HTML before the script loads
Local override — TSNav.init({...})

Verified: syntax clean, all 17 config checks pass, and the merge layering + legacy-flag folding simulated correctly. As always it's not browser-run — the live thing to confirm is just that your HTML sets window.TSNavConfig before the <script> tag if you use the global path (a defer script still executes after inline <script> in the head, so that ordering holds).

Feature flags: theme, dynamicNav, mobileMenu, collapse, mediaTokenize, cssBridge, backToTop, logoFit, logoFitInShowcase, upgradeLegacyThemeToggles.
Bucket overrides (deep-merged into the registries): selectors, ids, stateClasses, tokens, tokenDefaults.
*/ 
