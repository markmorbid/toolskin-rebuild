...
let   ROOT    = process.cwd();   // folder to SCAN and to serve previewed files from
let   APP_DIR = __dirname;       // folder holding the HTML + the data JSON
let   PORT    = 8080;

for (let i = 0; i < args.length; i++) {
  if      (args[i] === '--port' && args[i + 1]) PORT = parseInt(args[++i], 10);
  else if (args[i] === '--app'  && args[i + 1]) APP_DIR = path.resolve(args[++i]);
  else if (!args[i].startsWith('--'))           ROOT = path.resolve(args[i]);
}

// Outputs + the "latest" copies live alongside the HTML (the APP_DIR), so the
// page's relative fetch('./tree-explorer.data.json') always resolves.
const OUTPUT_DIR   = path.join(APP_DIR, '_treemap-output');
const DATA_LATEST  = path.join(APP_DIR, 'tree-explorer.data.json');
const DEDUP_LATEST = path.join(APP_DIR, 'tree-explorer.dedup.json');

const EXCLUDE = new Set([
  '.git', 'node_modules', '__pycache__', '.venv', 'venv',
  '_tree_output', '_treemap-output', '_tree-explorer', '_bu', '.next', 'dist', '.cache',
]);
...
async function handler(req, res) {
  ...
  // Static files: try the APP_DIR first (HTML, data JSON, app assets), then
  // fall back to the scanned ROOT (so previewed files resolve). Both are
  // confined to their base to block path traversal.
  let rel = decodeURIComponent(pathname);
  if (rel === '/') rel = '/tree-explorer.html';
  const safeRel = path.normalize(rel).replace(/^(\.\.[\/\\])+/, '');

  const appPath  = path.join(APP_DIR, safeRel);
  const rootPath = path.join(ROOT, safeRel);

  let filePath = null;
  if (appPath.indexOf(APP_DIR) === 0 && fs.existsSync(appPath) && !fs.statSync(appPath).isDirectory()) {
    filePath = appPath;
  } else if (rootPath.indexOf(ROOT) === 0 && fs.existsSync(rootPath) && !fs.statSync(rootPath).isDirectory()) {
    filePath = rootPath;
  }
...
http.createServer((req, res) => {
  ...
  console.log('  URL:     http://localhost:' + PORT + '/tree-explorer.html');
