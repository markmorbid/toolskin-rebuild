...
- **Typography primitives** (`--ts-fs-*` harmonic 1.125 ladder from **13px base**, `--ts-font-*`) — out of scope, in `primitives/typography.css` per T2 §3.2 row 3. **Wave 1.6 amendment (2026-05-20):** base = 15px (confirmed in running toolskin.css — NOT 13px, NOT 16px). **RULING 3 correction (2026-05-21):** 13px confirmed from `toolskin.css:287` — the Wave 1.6 amendment above is superseded; base = **13px**. Wave 1.6 visual audit (15px) was an instrumentation error. Weight ladder = `300/400/500/600/700/900` — **NO 800** (Space Grotesk ships 300–700 standard; OQ-B3 abandoned). H1=700, H2=600. Google Fonts `wght@300..700` URL.
