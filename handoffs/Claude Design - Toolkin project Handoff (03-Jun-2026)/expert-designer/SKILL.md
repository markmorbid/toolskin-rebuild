...
## 12a · Token-recursive design patterns (from nav v3 extraction)

These patterns encode what was learned building the nav to production-quality. Apply them to every component.

### Pattern A — Single-knob control
One token controls an entire visual dimension. All derived values reference it.
```css
--ts-{component}-text-alpha: 50%;
--ts-{component}-color: color-mix(in oklab, var(--ts-text-primary), transparent var(--ts-{component}-text-alpha));
```
Change the knob; every ink value updates. Never hardcode a secondary color — derive it from primary + alpha.

### Pattern B — Harmonic sizing from one anchor
All related geometry tokens derive from one root dimension.
```css
--ts-{component}-item-w:    calc(var(--ts-anchor-h) - var(--ts-pad));
--ts-{component}-item-h:    calc(var(--ts-anchor-h) - 1px);   /* -1px clears border */
--ts-{component}-item-icon: calc(var(--ts-{component}-item-w) * 0.5);
```
Change the anchor; everything rescales harmonically.

### Pattern C — Auto-inverting color via on-surface
```css
--ts-{component}-border: color-mix(in oklab, var(--ts-on-surface-auto), transparent N%);
```
`--ts-on-surface-auto` resolves correctly on dark, light, accent — zero per-surface overrides. Use oklab. The srgb border system is an owner-documented exception for the border scale tokens only.

### Pattern D — Constrained radius cap
```css
--ts-radius-constrained: min(var(--ts-card-radius, var(--ts-radius-sm)), var(--ts-radius-cap-raw));
```
Prevents a large global `--ts-radius-base` from breaking dense bar UI. Full-height controls get `border-radius: 0`. Pill controls use the constrained value.

### Pattern E — Set `--ts-this-bg` once, cascade everything
```css
.my-component { --ts-this-bg: var(--ts-bg-body); }
```
One line. Every `--ts-this-bg-*` engine derivative resolves from it automatically. To re-theme a subtree: override `--ts-this-bg` on the parent. Never set `background-color` directly — set the token.

### Pattern F — Nested `&` discipline
ALL states, variants, sub-elements, and pseudo-elements belong INSIDE their parent block. No scattered flat selectors:
```css
.my-component {
  color: var(--ts-text);

  &:hover { color: var(--ts-text-primary); }
  &.active { color: var(--ts-accent); }
  & .my-icon { font-size: inherit; }
}
```

### Pattern G — Derivative spacing chain
Define multipliers; derive all padding/gap from them:
```css
--ts-{c}-mult-lg: 1.5;
--ts-{c}-pad-x-mult: calc(var(--ts-{c}-pad-x) * var(--ts-{c}-mult-lg));
```
Change one multiplier → the whole component rescales.

### Hard rules (apply these without exception)
- `at-top` / scroll signals = ONLY signal state; visual transparency is a separate opt-in class
- Motion tokens are COMPONENT-SCOPED via `:where()`, never on `:root`  
- Zero `!important` in authored output
- Zero hardcoded hex — every color is `var(--ts-*)`
- oklab color-mix everywhere except srgb border system (documented owner exception)
- Progressive-enhancement pattern: JS-driven state classes gate CSS `@media` fallbacks via `[data-synced]` attributes
