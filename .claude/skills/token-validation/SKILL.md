...
> ⚠️ **REBUILD NOTE:** Token examples in this skill use the HSL model (`--ts-accent-h/s/l`). In toolskin-rebuild, ALL color tokens are OKLCH/apcach-derived per Extended Rule 15. The validation methodology (3-tier naming, reference scanning, composite documentation) is correct and binding. Token VALUE examples are stale — consult `_rebuild-primitives-spec.md` for current token names and values.
