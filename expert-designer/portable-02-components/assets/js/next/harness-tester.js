...
(function () {
  ...
  function optionGroup(legend, opts, current, onPick) {
    ...
    opts.forEach(function (o) {
      ...
      var isSwatch = (o.id === "orange" || ACCENTS.indexOf(o) > -1);

      var b = el("button", "ts-tester__opt");
      b.type = "button";

      if (isSwatch) {
        b.classList.add("opt_swatch");

        var inner = el("span", "ts-tester__opt-inner");
        inner.appendChild(el("span", "ts-tester__opt-swatch", o.label));
        inner.appendChild(document.createTextNode("")); // keep structure clean
        b.appendChild(inner);
      } else {
        b.textContent = o.label;
      }
      if (isSwatch) b.style.color = o.v;

      //end change mande
      if (o.id === "orange" || ACCENTS.indexOf(o) > -1) b.style.color = o.v; 
      // accent swatch tint
      ...
      g.classList.add("opt_modified"); // ← ONLY user interaction marks group
  ...
  function build() {
    ...
    var title = el("span", "ts-tester__title");
    title.appendChild(el("span", "ts-tester__icon", "◧"));
    title.appendChild(document.createTextNode(" Variant tester"));
    ...
    reset.addEventListener("click", function () {
      ...
      document.querySelectorAll(".ts-tester__group.opt_modified")
        .forEach(function (g) { g.classList.remove("opt_modified"); });
