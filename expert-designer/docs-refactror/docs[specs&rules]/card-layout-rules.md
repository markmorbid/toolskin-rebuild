...
> This document is the SINGLE SOURCE OF TRUTH for card structure in the offcanvas editor, or any Toolpanel/dashboard/unout  card-grouped asset tyes , originally built  under the  Banner gernertor, patter generatro, andOffancas Sidebar toolñ  requirements  design pattern.
Now the apporachs shoul be to:

1- make a @container type AND class container "ts-toolcards" (an auto  enabed aut appendable class  default, automatic class apeding  that dshpuld be set  as  uru by defailt, and off  by user n a visible place n the script - init, o he  tp confir of that class and also on generla config or asste config)  
to for  sidebars/panels type coainers to ths class  shuld be usable and valid  anywhere where 
